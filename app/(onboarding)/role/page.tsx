...
export default function RolePage() {
  ...
  return (
    <div className="flex-1 flex flex-col justify-center px-6">
      <h2 className="text-xl font-bold text-center mb-1 text-navy">Are you here to</h2>
      <p className="text-center text-sm mb-8 text-sub">You can't switch this later, so pick the one that fits.</p>
      <div className="space-y-3">
        <button onClick={() => pick("orderer")} className="w-full rounded-2xl p-5 text-left border-2 border-line bg-card active:scale-[0.98]">
          <span className="font-semibold text-base text-navy">Order</span>
          <p className="text-xs mt-1 text-sub">Get food or items brought to you</p>
        </button>
        <button onClick={() => pick("runner")} className="w-full rounded-2xl p-5 text-left border-2 border-amber bg-amber/10 active:scale-[0.98]">
          <span className="font-semibold text-base text-navy">Deliver</span>
          <p className="text-xs mt-1 text-sub">Register as a runner and start earning</p>
        </button>
      </div>
    </div>
  );
