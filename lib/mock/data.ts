...
// MOCK_CUSTOMERS, VENDORS, DESTINATIONS and seedChats() — same names, same
...
function customer(id: string, code: string, name: string, extra?: Partial<Person>): Person {
  return {
    id, code, name, type: "customer", campusId: "abuad",
...
export const MOCK_CUSTOMERS: Person[] = [
  customer("o1", "552013", "Tolu Bankole", { starredRunnerIds: ["r1", "r5"], walletBalance: 2000 }),
  customer("o2", "890144", "Bisi Fashola"),
  customer("o3", "220091", "Emeka Uche"),
  customer("o4", "771002", "Ngozi Eze"),
  customer("o5", "410358", "Zainab Yusuf"),
  customer("o6", "663017", "Adaeze Obi"),
];
...
function textMsg(chatId: string, sender: "runner" | "customer", text: string, at: number): Message {
...
export function seedChats(): Record<string, Chat> {
  ...
  chats[c1] = { id: c1, runnerId: "r1", customerId: "o1", unreadForRunner: 1, unreadForCustomer: 0, countdownEndsAt: null, lockedUntil: null, prefill: null, messages: [req1, textMsg(c1, "runner", "Sure, on it now — let me check the price", now - 8 * min + 10000), price1a, textMsg(c1, "customer", "That's a bit much for just rice and a drink — can you do 3000?", now - 7 * min), price1b, textMsg(c1, "customer", "Deal, sending payment now", now - 3 * min - 20000)] };
  ...
  chats[c2] = { id: c2, runnerId: "r1", customerId: "o2", unreadForRunner: 0, unreadForCustomer: 0, countdownEndsAt: null, lockedUntil: null, prefill: null, messages: [textMsg(c2, "customer", "Good morning, I want to order suya from Cafe One please", now - 70 * min), textMsg(c2, "runner", "Morning! What quantity, and any extra spice?", now - 65 * min), req2, price2, textMsg(c2, "customer", "Paid! On my way to class but I'll be back by 12", now - 39 * min), textMsg(c2, "runner", "No wahala, I'll hold it till you're back", now - 38 * min)] };
  ...
  chats[c3] = { id: c3, runnerId: "r1", customerId: "o3", unreadForRunner: 0, unreadForCustomer: 0, countdownEndsAt: null, lockedUntil: null, prefill: null, messages: [textMsg(c3, "customer", "Hi Kemi, are you free to run an order this evening?", now - 26 * hr), textMsg(c3, "runner", "Okay boss, send the details", now - 26 * hr + 2 * min), req3, price3, textMsg(c3, "customer", "Paid already, see you by 7", now - 25 * hr - 50 * min), systemMsg(c3, "Order fulfilled — code confirmed on delivery.", now - 25 * hr), textMsg(c3, "customer", "Got it, thank you so much!", now - 25 * hr + 2 * min), textMsg(c3, "runner", "You're welcome, enjoy!", now - 25 * hr + 3 * min)] };
  ...
  chats[c5] = { id: c5, runnerId: "r1", customerId: "o5", unreadForRunner: 0, unreadForCustomer: 0, countdownEndsAt: null, lockedUntil: null, prefill: null, messages: [textMsg(c5, "customer", "Hi, please can I get something from Cafe One?", now - 5 * hr), textMsg(c5, "runner", "Sure! Delivery or pickup?", now - 5 * hr + 2 * min), req5, price5, textMsg(c5, "customer", "Just paid, thank you!", now - 4 * hr + 1 * min), textMsg(c5, "runner", "Got it, on my way shortly", now - 4 * hr + 2 * min)] };
  ...
  chats[c4] = { id: c4, runnerId: "r1", customerId: "o4", unreadForRunner: 0, unreadForCustomer: 0, countdownEndsAt: null, lockedUntil: now + 23 * hr, prefill: null, messages: [req4, systemMsg(c4, "This chat is locked because there was no response within 5 minutes. It will reopen in 24 hours.", now - hr)] };
  ...
  chats[c8] = { id: c8, runnerId: "r1", customerId: "o6", unreadForRunner: 1, unreadForCustomer: 0, countdownEndsAt: now + 3 * min + 30000, lockedUntil: null, prefill: null, messages: [req8] };
  ...
  chats[c6] = { id: c6, runnerId: "r2", customerId: "o1", unreadForRunner: 0, unreadForCustomer: 0, countdownEndsAt: null, lockedUntil: null, prefill: null, messages: [textMsg(c6, "customer", "Hi Chidi, can I get meat pie and zobo from Bites and Eats?", now - 2 * day), textMsg(c6, "runner", "Yes ma, gimme 2 mins to confirm price", now - 2 * day + 2 * min), req6, price6, textMsg(c6, "customer", "Paid! I'll be at the hostel gate by 6", now - 2 * day + 7 * min), systemMsg(c6, "Order fulfilled — code confirmed on delivery.", now - 2 * day + 8 * min), textMsg(c6, "customer", "Thanks for being quick as usual", now - 2 * day + 9 * min), textMsg(c6, "runner", "Anytime! Message me whenever", now - 2 * day + 10 * min)] };
  ...
  chats[c7] = { id: c7, runnerId: "r3", customerId: "o1", unreadForRunner: 0, unreadForCustomer: 0, countdownEndsAt: null, lockedUntil: null, prefill: null, messages: [textMsg(c7, "customer", "Do you have smoothies in stock? Want a large mango one", now - 22 * hr), textMsg(c7, "runner", "Yes! It's pickup only though, can you come to Smoothie Bar?", now - 22 * hr + 3 * min), req7, price7, textMsg(c7, "customer", "Paid, heading there now", now - 21 * hr + 2 * min), textMsg(c7, "runner", "See you soon", now - 21 * hr + 3 * min)] };
